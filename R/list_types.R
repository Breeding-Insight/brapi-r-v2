...
#' @export
ListTypes <- R6::R6Class(
    "ListTypes",
    public = list(
        initialize = function(...) {
            local.optional.var <- list(...)
            val <- unlist(local.optional.var)
            enumvec <- .parse_ListTypes()
        ...
        fromJSON = function(ListTypesJson) {
            private$value <- jsonlite::fromJSON(ListTypesJson,
        ...
        fromJSONString = function(ListTypesJson) {
            private$value <- jsonlite::fromJSON(ListTypesJson,
                simplifyVector = FALSE)
            self
        }
...
# add to utils.R
.parse_ListTypes <- function(vals) {
    res <- gsub("^\\[|\\]$", "",
        "[germplasm, markers, programs, trials, studies, observationUnits, observations, observationVariables, samples]"
    )
    unlist(strsplit(res, ", "))
}
