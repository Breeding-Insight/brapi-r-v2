...
#'
#' @description ObservationSingleResponse Class
#'
...
#'
#' @field result  \link{Observation} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
ObservationSingleResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      ObservationSingleResponseObject <- list()
      if (!is.null(self$`@context`)) {
        ObservationSingleResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        ObservationSingleResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        ObservationSingleResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      ObservationSingleResponseObject
    },
    fromJSON = function(ObservationSingleResponseJson) {
      ObservationSingleResponseObject <- jsonlite::fromJSON(ObservationSingleResponseJson)
      if (!is.null(ObservationSingleResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(ObservationSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(ObservationSingleResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(ObservationSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(ObservationSingleResponseObject$`result`)) {
        resultObject <- Observation$new()
        resultObject$fromJSON(jsonlite::toJSON(ObservationSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(ObservationSingleResponseJson) {
      ObservationSingleResponseObject <- jsonlite::fromJSON(ObservationSingleResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(ObservationSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(ObservationSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- Observation$new()$fromJSON(jsonlite::toJSON(ObservationSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
