...
#'
#' @description TraitSingleResponse Class
#'
...
#'
#' @field result  \link{Trait} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
TraitSingleResponse <- R6::R6Class(
  'TraitSingleResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      TraitSingleResponseObject <- list()
      if (!is.null(self$`@context`)) {
        TraitSingleResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        TraitSingleResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        TraitSingleResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      TraitSingleResponseObject
    },
    fromJSON = function(TraitSingleResponseJson) {
      TraitSingleResponseObject <- jsonlite::fromJSON(TraitSingleResponseJson)
      if (!is.null(TraitSingleResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(TraitSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(TraitSingleResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(TraitSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(TraitSingleResponseObject$`result`)) {
        resultObject <- Trait$new()
        resultObject$fromJSON(jsonlite::toJSON(TraitSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(TraitSingleResponseJson) {
      TraitSingleResponseObject <- jsonlite::fromJSON(TraitSingleResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(TraitSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(TraitSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- Trait$new()$fromJSON(jsonlite::toJSON(TraitSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
