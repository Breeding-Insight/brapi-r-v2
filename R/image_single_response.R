...
#'
#' @description ImageSingleResponse Class
#'
...
#'
#' @field result  \link{Image} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
ImageSingleResponse <- R6::R6Class(
  'ImageSingleResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      ImageSingleResponseObject <- list()
      if (!is.null(self$`@context`)) {
        ImageSingleResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        ImageSingleResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        ImageSingleResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      ImageSingleResponseObject
    },
    fromJSON = function(ImageSingleResponseJson) {
      ImageSingleResponseObject <- jsonlite::fromJSON(ImageSingleResponseJson)
      if (!is.null(ImageSingleResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(ImageSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(ImageSingleResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(ImageSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(ImageSingleResponseObject$`result`)) {
        resultObject <- Image$new()
        resultObject$fromJSON(jsonlite::toJSON(ImageSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(ImageSingleResponseJson) {
      ImageSingleResponseObject <- jsonlite::fromJSON(ImageSingleResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(ImageSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(ImageSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- Image$new()$fromJSON(jsonlite::toJSON(ImageSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
