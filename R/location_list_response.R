...
#'
#' @description LocationListResponse Class
#'
...
#'
#' @field result  \link{LocationListResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
LocationListResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      LocationListResponseObject <- list()
      if (!is.null(self$`@context`)) {
        LocationListResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        LocationListResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        LocationListResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      LocationListResponseObject
    },
    fromJSON = function(LocationListResponseJson) {
      LocationListResponseObject <- jsonlite::fromJSON(LocationListResponseJson)
      if (!is.null(LocationListResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(LocationListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(LocationListResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(LocationListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(LocationListResponseObject$`result`)) {
        resultObject <- LocationListResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(LocationListResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(LocationListResponseJson) {
      LocationListResponseObject <- jsonlite::fromJSON(LocationListResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(LocationListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(LocationListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- LocationListResponseResult$new()$fromJSON(jsonlite::toJSON(LocationListResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
