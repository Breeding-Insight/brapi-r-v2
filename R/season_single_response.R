...
#'
#' @description SeasonSingleResponse Class
#'
...
#'
#' @field result  \link{Season} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
SeasonSingleResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      SeasonSingleResponseObject <- list()
      if (!is.null(self$`@context`)) {
        SeasonSingleResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        SeasonSingleResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        SeasonSingleResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      SeasonSingleResponseObject
    },
    fromJSON = function(SeasonSingleResponseJson) {
      SeasonSingleResponseObject <- jsonlite::fromJSON(SeasonSingleResponseJson)
      if (!is.null(SeasonSingleResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(SeasonSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(SeasonSingleResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(SeasonSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(SeasonSingleResponseObject$`result`)) {
        resultObject <- Season$new()
        resultObject$fromJSON(jsonlite::toJSON(SeasonSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(SeasonSingleResponseJson) {
      SeasonSingleResponseObject <- jsonlite::fromJSON(SeasonSingleResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(SeasonSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(SeasonSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- Season$new()$fromJSON(jsonlite::toJSON(SeasonSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
