...
#'
#' @description StudyListResponse Class
#'
...
#'
#' @field result  \link{StudyListResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
StudyListResponse <- R6::R6Class(
  'StudyListResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      StudyListResponseObject <- list()
      if (!is.null(self$`@context`)) {
        StudyListResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        StudyListResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        StudyListResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      StudyListResponseObject
    },
    fromJSON = function(StudyListResponseJson) {
      StudyListResponseObject <- jsonlite::fromJSON(StudyListResponseJson)
      if (!is.null(StudyListResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(StudyListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(StudyListResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(StudyListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(StudyListResponseObject$`result`)) {
        resultObject <- StudyListResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(StudyListResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(StudyListResponseJson) {
      StudyListResponseObject <- jsonlite::fromJSON(StudyListResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(StudyListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(StudyListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- StudyListResponseResult$new()$fromJSON(jsonlite::toJSON(StudyListResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
