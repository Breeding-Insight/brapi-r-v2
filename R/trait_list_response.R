...
#'
#' @description TraitListResponse Class
#'
...
#'
#' @field result  \link{TraitListResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
TraitListResponse <- R6::R6Class(
  'TraitListResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      TraitListResponseObject <- list()
      if (!is.null(self$`@context`)) {
        TraitListResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        TraitListResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        TraitListResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      TraitListResponseObject
    },
    fromJSON = function(TraitListResponseJson) {
      TraitListResponseObject <- jsonlite::fromJSON(TraitListResponseJson)
      if (!is.null(TraitListResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(TraitListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(TraitListResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(TraitListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(TraitListResponseObject$`result`)) {
        resultObject <- TraitListResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(TraitListResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(TraitListResponseJson) {
      TraitListResponseObject <- jsonlite::fromJSON(TraitListResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(TraitListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(TraitListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- TraitListResponseResult$new()$fromJSON(jsonlite::toJSON(TraitListResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
