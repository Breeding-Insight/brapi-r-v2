...
#'
#' @description PersonListResponse Class
#'
...
#'
#' @field result  \link{PersonListResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
PersonListResponse <- R6::R6Class(
  'PersonListResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      PersonListResponseObject <- list()
      if (!is.null(self$`@context`)) {
        PersonListResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        PersonListResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        PersonListResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      PersonListResponseObject
    },
    fromJSON = function(PersonListResponseJson) {
      PersonListResponseObject <- jsonlite::fromJSON(PersonListResponseJson)
      if (!is.null(PersonListResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(PersonListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(PersonListResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(PersonListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(PersonListResponseObject$`result`)) {
        resultObject <- PersonListResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(PersonListResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(PersonListResponseJson) {
      PersonListResponseObject <- jsonlite::fromJSON(PersonListResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(PersonListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(PersonListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- PersonListResponseResult$new()$fromJSON(jsonlite::toJSON(PersonListResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
