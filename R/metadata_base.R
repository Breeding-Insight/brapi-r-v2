...
#' @title MetadataBase
#'
#' @description MetadataBase Class
#'
...
#' @export
MetadataBase <- R6::R6Class(
  'MetadataBase',
  public = list(
    `datafiles` = NULL,
    `status` = NULL,
    initialize = function(
        `datafiles`=NULL, `status`=NULL, ...
    ) {
    ...
    toJSON = function() {
      MetadataBaseObject <- list()
      if (!is.null(self$`datafiles`)) {
        MetadataBaseObject[['datafiles']] <-
          lapply(self$`datafiles`, function(x) x$toJSON())
      }
      if (!is.null(self$`status`)) {
        MetadataBaseObject[['status']] <-
          lapply(self$`status`, function(x) x$toJSON())
      }

      MetadataBaseObject
    },
    fromJSON = function(MetadataBaseJson) {
      MetadataBaseObject <- jsonlite::fromJSON(MetadataBaseJson)
      if (!is.null(MetadataBaseObject$`datafiles`)) {
        self$`datafiles` <- ApiClient$new()$deserializeObj(MetadataBaseObject$`datafiles`, "array[DataFile]", loadNamespace("openapi"))
      }
      if (!is.null(MetadataBaseObject$`status`)) {
        self$`status` <- ApiClient$new()$deserializeObj(MetadataBaseObject$`status`, "array[Status]", loadNamespace("openapi"))
      }
      self
    },
...
',
    ...
    fromJSONString = function(MetadataBaseJson) {
      MetadataBaseObject <- jsonlite::fromJSON(MetadataBaseJson)
      self$`datafiles` <- ApiClient$new()$deserializeObj(MetadataBaseObject$`datafiles`, "array[DataFile]", loadNamespace("openapi"))
      self$`status` <- ApiClient$new()$deserializeObj(MetadataBaseObject$`status`, "array[Status]", loadNamespace("openapi"))
      self
