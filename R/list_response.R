  ...
  public = list(
    ...
    toJSON = function() {
      ...
      if (!is.null(self$`@context`)) {
        ListResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        ListResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        ListResponseObject[['result']] <-
          self$`result`$toJSON()
      }
    ...
    fromJSON = function(ListResponseJson) {
      ...
      if (!is.null(ListResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(ListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(ListResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(ListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(ListResponseObject$`result`)) {
        resultObject <- ListDetails$new()
        resultObject$fromJSON(jsonlite::toJSON(ListResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(ListResponseJson) {
      ListResponseObject <- jsonlite::fromJSON(ListResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(ListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(ListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- ListDetails$new()$fromJSON(jsonlite::toJSON(ListResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
