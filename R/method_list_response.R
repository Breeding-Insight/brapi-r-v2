...
#'
#' @description MethodListResponse Class
#'
...
#'
#' @field result  \link{MethodListResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
MethodListResponse <- R6::R6Class(
  'MethodListResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      MethodListResponseObject <- list()
      if (!is.null(self$`@context`)) {
        MethodListResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        MethodListResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        MethodListResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      MethodListResponseObject
    },
    fromJSON = function(MethodListResponseJson) {
      MethodListResponseObject <- jsonlite::fromJSON(MethodListResponseJson)
      if (!is.null(MethodListResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(MethodListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(MethodListResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(MethodListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(MethodListResponseObject$`result`)) {
        resultObject <- MethodListResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(MethodListResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(MethodListResponseJson) {
      MethodListResponseObject <- jsonlite::fromJSON(MethodListResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(MethodListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(MethodListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- MethodListResponseResult$new()$fromJSON(jsonlite::toJSON(MethodListResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
