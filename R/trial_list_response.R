...
#'
#' @description TrialListResponse Class
#'
...
#'
#' @field result  \link{TrialListResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
TrialListResponse <- R6::R6Class(
  'TrialListResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      TrialListResponseObject <- list()
      if (!is.null(self$`@context`)) {
        TrialListResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        TrialListResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        TrialListResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      TrialListResponseObject
    },
    fromJSON = function(TrialListResponseJson) {
      TrialListResponseObject <- jsonlite::fromJSON(TrialListResponseJson)
      if (!is.null(TrialListResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(TrialListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(TrialListResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(TrialListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(TrialListResponseObject$`result`)) {
        resultObject <- TrialListResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(TrialListResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(TrialListResponseJson) {
      TrialListResponseObject <- jsonlite::fromJSON(TrialListResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(TrialListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(TrialListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- TrialListResponseResult$new()$fromJSON(jsonlite::toJSON(TrialListResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
