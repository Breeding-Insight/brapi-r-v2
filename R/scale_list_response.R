...
#'
#' @description ScaleListResponse Class
#'
...
#'
#' @field result  \link{ScaleListResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
ScaleListResponse <- R6::R6Class(
  'ScaleListResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      ScaleListResponseObject <- list()
      if (!is.null(self$`@context`)) {
        ScaleListResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        ScaleListResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        ScaleListResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      ScaleListResponseObject
    },
    fromJSON = function(ScaleListResponseJson) {
      ScaleListResponseObject <- jsonlite::fromJSON(ScaleListResponseJson)
      if (!is.null(ScaleListResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(ScaleListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(ScaleListResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(ScaleListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(ScaleListResponseObject$`result`)) {
        resultObject <- ScaleListResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(ScaleListResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(ScaleListResponseJson) {
      ScaleListResponseObject <- jsonlite::fromJSON(ScaleListResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(ScaleListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(ScaleListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- ScaleListResponseResult$new()$fromJSON(jsonlite::toJSON(ScaleListResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
