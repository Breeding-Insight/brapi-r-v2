...
#'
#' @description CallSetsListResponse Class
#'
...
#'
#' @field result  \link{CallSetsListResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
CallSetsListResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      CallSetsListResponseObject <- list()
      if (!is.null(self$`@context`)) {
        CallSetsListResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        CallSetsListResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        CallSetsListResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      CallSetsListResponseObject
    },
    fromJSON = function(CallSetsListResponseJson) {
      CallSetsListResponseObject <- jsonlite::fromJSON(CallSetsListResponseJson)
      if (!is.null(CallSetsListResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(CallSetsListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(CallSetsListResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(CallSetsListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(CallSetsListResponseObject$`result`)) {
        resultObject <- CallSetsListResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(CallSetsListResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(CallSetsListResponseJson) {
      CallSetsListResponseObject <- jsonlite::fromJSON(CallSetsListResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(CallSetsListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(CallSetsListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- CallSetsListResponseResult$new()$fromJSON(jsonlite::toJSON(CallSetsListResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
