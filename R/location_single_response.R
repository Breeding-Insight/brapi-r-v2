...
#'
#' @description LocationSingleResponse Class
#'
...
#'
#' @field result  \link{Location} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
LocationSingleResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      LocationSingleResponseObject <- list()
      if (!is.null(self$`@context`)) {
        LocationSingleResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        LocationSingleResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        LocationSingleResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      LocationSingleResponseObject
    },
    fromJSON = function(LocationSingleResponseJson) {
      LocationSingleResponseObject <- jsonlite::fromJSON(LocationSingleResponseJson)
      if (!is.null(LocationSingleResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(LocationSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(LocationSingleResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(LocationSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(LocationSingleResponseObject$`result`)) {
        resultObject <- Location$new()
        resultObject$fromJSON(jsonlite::toJSON(LocationSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(LocationSingleResponseJson) {
      LocationSingleResponseObject <- jsonlite::fromJSON(LocationSingleResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(LocationSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(LocationSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- Location$new()$fromJSON(jsonlite::toJSON(LocationSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
