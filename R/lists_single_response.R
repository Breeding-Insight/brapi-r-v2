...
#'
#' @description ListsSingleResponse Class
#'
...
#'
#' @field result  \link{ListDetails} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
ListsSingleResponse <- R6::R6Class(
  'ListsSingleResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      ListsSingleResponseObject <- list()
      if (!is.null(self$`@context`)) {
        ListsSingleResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        ListsSingleResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        ListsSingleResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      ListsSingleResponseObject
    },
    fromJSON = function(ListsSingleResponseJson) {
      ListsSingleResponseObject <- jsonlite::fromJSON(ListsSingleResponseJson)
      if (!is.null(ListsSingleResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(ListsSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(ListsSingleResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(ListsSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(ListsSingleResponseObject$`result`)) {
        resultObject <- ListDetails$new()
        resultObject$fromJSON(jsonlite::toJSON(ListsSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(ListsSingleResponseJson) {
      ListsSingleResponseObject <- jsonlite::fromJSON(ListsSingleResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(ListsSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(ListsSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- ListDetails$new()$fromJSON(jsonlite::toJSON(ListsSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
