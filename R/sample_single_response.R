...
#'
#' @description SampleSingleResponse Class
#'
...
#'
#' @field result  \link{Sample} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
SampleSingleResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      SampleSingleResponseObject <- list()
      if (!is.null(self$`@context`)) {
        SampleSingleResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        SampleSingleResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        SampleSingleResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      SampleSingleResponseObject
    },
    fromJSON = function(SampleSingleResponseJson) {
      SampleSingleResponseObject <- jsonlite::fromJSON(SampleSingleResponseJson)
      if (!is.null(SampleSingleResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(SampleSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(SampleSingleResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(SampleSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(SampleSingleResponseObject$`result`)) {
        resultObject <- Sample$new()
        resultObject$fromJSON(jsonlite::toJSON(SampleSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(SampleSingleResponseJson) {
      SampleSingleResponseObject <- jsonlite::fromJSON(SampleSingleResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(SampleSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(SampleSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- Sample$new()$fromJSON(jsonlite::toJSON(SampleSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
