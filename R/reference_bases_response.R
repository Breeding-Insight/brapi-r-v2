...
#'
#' @description ReferenceBasesResponse Class
#'
...
#'
#' @field result  \link{ReferenceBases} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
ReferenceBasesResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      ReferenceBasesResponseObject <- list()
      if (!is.null(self$`@context`)) {
        ReferenceBasesResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        ReferenceBasesResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        ReferenceBasesResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      ReferenceBasesResponseObject
    },
    fromJSON = function(ReferenceBasesResponseJson) {
      ReferenceBasesResponseObject <- jsonlite::fromJSON(ReferenceBasesResponseJson)
      if (!is.null(ReferenceBasesResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(ReferenceBasesResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(ReferenceBasesResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(ReferenceBasesResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(ReferenceBasesResponseObject$`result`)) {
        resultObject <- ReferenceBases$new()
        resultObject$fromJSON(jsonlite::toJSON(ReferenceBasesResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(ReferenceBasesResponseJson) {
      ReferenceBasesResponseObject <- jsonlite::fromJSON(ReferenceBasesResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(ReferenceBasesResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(ReferenceBasesResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- ReferenceBases$new()$fromJSON(jsonlite::toJSON(ReferenceBasesResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
