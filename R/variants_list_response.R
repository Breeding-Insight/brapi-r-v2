...
#'
#' @description VariantsListResponse Class
#'
#' @format An \code{R6Class} generator object
#'
#' @field @context context list( character ) [optional]
#'
#' @field metadata  \link{MetadataTokenPagination} 
#'
#' @field result  \link{VariantsListResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
VariantsListResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      VariantsListResponseObject <- list()
      if (!is.null(self$`@context`)) {
        VariantsListResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        VariantsListResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        VariantsListResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      VariantsListResponseObject
    },
    fromJSON = function(VariantsListResponseJson) {
      VariantsListResponseObject <- jsonlite::fromJSON(VariantsListResponseJson)
      if (!is.null(VariantsListResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(VariantsListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(VariantsListResponseObject$`metadata`)) {
        metadataObject <- MetadataTokenPagination$new()
        metadataObject$fromJSON(jsonlite::toJSON(VariantsListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(VariantsListResponseObject$`result`)) {
        resultObject <- VariantsListResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(VariantsListResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(VariantsListResponseJson) {
      VariantsListResponseObject <- jsonlite::fromJSON(VariantsListResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(VariantsListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- MetadataTokenPagination$new()$fromJSON(jsonlite::toJSON(VariantsListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- VariantsListResponseResult$new()$fromJSON(jsonlite::toJSON(VariantsListResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
