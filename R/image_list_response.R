...
#'
#' @description ImageListResponse Class
#'
...
#'
#' @field result  \link{ImageListResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
ImageListResponse <- R6::R6Class(
  'ImageListResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      ImageListResponseObject <- list()
      if (!is.null(self$`@context`)) {
        ImageListResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        ImageListResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        ImageListResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      ImageListResponseObject
    },
    fromJSON = function(ImageListResponseJson) {
      ImageListResponseObject <- jsonlite::fromJSON(ImageListResponseJson)
      if (!is.null(ImageListResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(ImageListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(ImageListResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(ImageListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(ImageListResponseObject$`result`)) {
        resultObject <- ImageListResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(ImageListResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(ImageListResponseJson) {
      ImageListResponseObject <- jsonlite::fromJSON(ImageListResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(ImageListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(ImageListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- ImageListResponseResult$new()$fromJSON(jsonlite::toJSON(ImageListResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
