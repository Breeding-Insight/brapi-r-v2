...
#' @title MetadataAllOf
#'
#' @description MetadataAllOf Class
#'
...
MetadataAllOf <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      MetadataAllOfObject <- list()
      if (!is.null(self$`pagination`)) {
        MetadataAllOfObject[['pagination']] <-
          self$`pagination`$toJSON()
      }

      MetadataAllOfObject
    },
    fromJSON = function(MetadataAllOfJson) {
      MetadataAllOfObject <- jsonlite::fromJSON(MetadataAllOfJson)
      if (!is.null(MetadataAllOfObject$`pagination`)) {
        paginationObject <- IndexPagination$new()
        paginationObject$fromJSON(jsonlite::toJSON(MetadataAllOfObject$pagination, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(MetadataAllOfJson) {
      MetadataAllOfObject <- jsonlite::fromJSON(MetadataAllOfJson)
      self$`pagination` <- IndexPagination$new()$fromJSON(jsonlite::toJSON(MetadataAllOfObject$pagination, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
