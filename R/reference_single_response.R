...
#'
#' @description ReferenceSingleResponse Class
#'
...
#'
#' @field result  \link{Reference} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
ReferenceSingleResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      ReferenceSingleResponseObject <- list()
      if (!is.null(self$`@context`)) {
        ReferenceSingleResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        ReferenceSingleResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        ReferenceSingleResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      ReferenceSingleResponseObject
    },
    fromJSON = function(ReferenceSingleResponseJson) {
      ReferenceSingleResponseObject <- jsonlite::fromJSON(ReferenceSingleResponseJson)
      if (!is.null(ReferenceSingleResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(ReferenceSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(ReferenceSingleResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(ReferenceSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(ReferenceSingleResponseObject$`result`)) {
        resultObject <- Reference$new()
        resultObject$fromJSON(jsonlite::toJSON(ReferenceSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(ReferenceSingleResponseJson) {
      ReferenceSingleResponseObject <- jsonlite::fromJSON(ReferenceSingleResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(ReferenceSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(ReferenceSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- Reference$new()$fromJSON(jsonlite::toJSON(ReferenceSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
