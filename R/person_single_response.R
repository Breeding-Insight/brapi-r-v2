...
#'
#' @description PersonSingleResponse Class
#'
...
#'
#' @field result  \link{Person} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
PersonSingleResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      PersonSingleResponseObject <- list()
      if (!is.null(self$`@context`)) {
        PersonSingleResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        PersonSingleResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        PersonSingleResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      PersonSingleResponseObject
    },
    fromJSON = function(PersonSingleResponseJson) {
      PersonSingleResponseObject <- jsonlite::fromJSON(PersonSingleResponseJson)
      if (!is.null(PersonSingleResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(PersonSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(PersonSingleResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(PersonSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(PersonSingleResponseObject$`result`)) {
        resultObject <- Person$new()
        resultObject$fromJSON(jsonlite::toJSON(PersonSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(PersonSingleResponseJson) {
      PersonSingleResponseObject <- jsonlite::fromJSON(PersonSingleResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(PersonSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(PersonSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- Person$new()$fromJSON(jsonlite::toJSON(PersonSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
