...
#'
#' @description CallsListResponse Class
#'
...
#'
#' @field result  \link{CallsListResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
CallsListResponse <- R6::R6Class(
  'CallsListResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      CallsListResponseObject <- list()
      if (!is.null(self$`@context`)) {
        CallsListResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        CallsListResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        CallsListResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      CallsListResponseObject
    },
    fromJSON = function(CallsListResponseJson) {
      CallsListResponseObject <- jsonlite::fromJSON(CallsListResponseJson)
      if (!is.null(CallsListResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(CallsListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(CallsListResponseObject$`metadata`)) {
        metadataObject <- MetadataTokenPagination$new()
        metadataObject$fromJSON(jsonlite::toJSON(CallsListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(CallsListResponseObject$`result`)) {
        resultObject <- CallsListResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(CallsListResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(CallsListResponseJson) {
      CallsListResponseObject <- jsonlite::fromJSON(CallsListResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(CallsListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- MetadataTokenPagination$new()$fromJSON(jsonlite::toJSON(CallsListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- CallsListResponseResult$new()$fromJSON(jsonlite::toJSON(CallsListResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
