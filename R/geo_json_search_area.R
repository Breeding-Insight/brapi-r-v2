...
#'
#' @description GeoJSONSearchArea Class
#'
#' @format An \code{R6Class} generator object
#'
#' @field geometry GeoJSON Geometry \link{OneOfpointGeometrypolygonGeometry} [optional]
#'
...
GeoJSONSearchArea <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      GeoJSONSearchAreaObject <- list()
      if (!is.null(self$`geometry`)) {
        GeoJSONSearchAreaObject[['geometry']] <-
          self$`geometry`$toJSON()
      }
      if (!is.null(self$`type`)) {
        GeoJSONSearchAreaObject[['type']] <-
          self$`type`
      }

      GeoJSONSearchAreaObject
    },
    fromJSON = function(GeoJSONSearchAreaJson) {
      GeoJSONSearchAreaObject <- jsonlite::fromJSON(GeoJSONSearchAreaJson)
      if (!is.null(GeoJSONSearchAreaObject$`geometry`)) {
        geometryObject <- OneOfpointGeometrypolygonGeometry$new()
        geometryObject$fromJSON(jsonlite::toJSON(GeoJSONSearchAreaObject$geometry, auto_unbox = TRUE, digits = NA))
        self$`geometry` <- geometryObject
      }
      if (!is.null(GeoJSONSearchAreaObject$`type`)) {
        self$`type` <- GeoJSONSearchAreaObject$`type`
      }
      self
    },
    ...
    fromJSONString = function(GeoJSONSearchAreaJson) {
      GeoJSONSearchAreaObject <- jsonlite::fromJSON(GeoJSONSearchAreaJson)
      self$`geometry` <- OneOfpointGeometrypolygonGeometry$new()$fromJSON(jsonlite::toJSON(GeoJSONSearchAreaObject$geometry, auto_unbox = TRUE, digits = NA))
      self$`type` <- GeoJSONSearchAreaObject$`type`
      self
    }
  )
