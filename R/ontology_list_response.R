...
#'
#' @description OntologyListResponse Class
#'
...
#'
#' @field result  \link{OntologyListResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
OntologyListResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      OntologyListResponseObject <- list()
      if (!is.null(self$`@context`)) {
        OntologyListResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        OntologyListResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        OntologyListResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      OntologyListResponseObject
    },
    fromJSON = function(OntologyListResponseJson) {
      OntologyListResponseObject <- jsonlite::fromJSON(OntologyListResponseJson)
      if (!is.null(OntologyListResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(OntologyListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(OntologyListResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(OntologyListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(OntologyListResponseObject$`result`)) {
        resultObject <- OntologyListResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(OntologyListResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(OntologyListResponseJson) {
      OntologyListResponseObject <- jsonlite::fromJSON(OntologyListResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(OntologyListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(OntologyListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- OntologyListResponseResult$new()$fromJSON(jsonlite::toJSON(OntologyListResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
