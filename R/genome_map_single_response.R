...
#'
#' @description GenomeMapSingleResponse Class
#'
...
#'
#' @field result  \link{GenomeMap} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
GenomeMapSingleResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      GenomeMapSingleResponseObject <- list()
      if (!is.null(self$`@context`)) {
        GenomeMapSingleResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        GenomeMapSingleResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        GenomeMapSingleResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      GenomeMapSingleResponseObject
    },
    fromJSON = function(GenomeMapSingleResponseJson) {
      GenomeMapSingleResponseObject <- jsonlite::fromJSON(GenomeMapSingleResponseJson)
      if (!is.null(GenomeMapSingleResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(GenomeMapSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(GenomeMapSingleResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(GenomeMapSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(GenomeMapSingleResponseObject$`result`)) {
        resultObject <- GenomeMap$new()
        resultObject$fromJSON(jsonlite::toJSON(GenomeMapSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(GenomeMapSingleResponseJson) {
      GenomeMapSingleResponseObject <- jsonlite::fromJSON(GenomeMapSingleResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(GenomeMapSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(GenomeMapSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- GenomeMap$new()$fromJSON(jsonlite::toJSON(GenomeMapSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
