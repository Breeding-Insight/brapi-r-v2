...
#'
#' @description ScaleSingleResponse Class
#'
...
#'
#' @field result  \link{Scale} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
ScaleSingleResponse <- R6::R6Class(
  'ScaleSingleResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      ScaleSingleResponseObject <- list()
      if (!is.null(self$`@context`)) {
        ScaleSingleResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        ScaleSingleResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        ScaleSingleResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      ScaleSingleResponseObject
    },
    fromJSON = function(ScaleSingleResponseJson) {
      ScaleSingleResponseObject <- jsonlite::fromJSON(ScaleSingleResponseJson)
      if (!is.null(ScaleSingleResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(ScaleSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(ScaleSingleResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(ScaleSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(ScaleSingleResponseObject$`result`)) {
        resultObject <- Scale$new()
        resultObject$fromJSON(jsonlite::toJSON(ScaleSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(ScaleSingleResponseJson) {
      ScaleSingleResponseObject <- jsonlite::fromJSON(ScaleSingleResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(ScaleSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(ScaleSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- Scale$new()$fromJSON(jsonlite::toJSON(ScaleSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
