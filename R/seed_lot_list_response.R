...
#'
#' @description SeedLotListResponse Class
#'
...
#'
#' @field result  \link{SeedLotListResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
SeedLotListResponse <- R6::R6Class(
  'SeedLotListResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      SeedLotListResponseObject <- list()
      if (!is.null(self$`@context`)) {
        SeedLotListResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        SeedLotListResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        SeedLotListResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      SeedLotListResponseObject
    },
    fromJSON = function(SeedLotListResponseJson) {
      SeedLotListResponseObject <- jsonlite::fromJSON(SeedLotListResponseJson)
      if (!is.null(SeedLotListResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(SeedLotListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(SeedLotListResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(SeedLotListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(SeedLotListResponseObject$`result`)) {
        resultObject <- SeedLotListResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(SeedLotListResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(SeedLotListResponseJson) {
      SeedLotListResponseObject <- jsonlite::fromJSON(SeedLotListResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(SeedLotListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(SeedLotListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- SeedLotListResponseResult$new()$fromJSON(jsonlite::toJSON(SeedLotListResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
