...
#'
#' @description VariantSetResponse Class
#'
...
#'
#' @field result  \link{VariantSet} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
VariantSetResponse <- R6::R6Class(
  'VariantSetResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      VariantSetResponseObject <- list()
      if (!is.null(self$`@context`)) {
        VariantSetResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        VariantSetResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        VariantSetResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      VariantSetResponseObject
    },
    fromJSON = function(VariantSetResponseJson) {
      VariantSetResponseObject <- jsonlite::fromJSON(VariantSetResponseJson)
      if (!is.null(VariantSetResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(VariantSetResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(VariantSetResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(VariantSetResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(VariantSetResponseObject$`result`)) {
        resultObject <- VariantSet$new()
        resultObject$fromJSON(jsonlite::toJSON(VariantSetResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(VariantSetResponseJson) {
      VariantSetResponseObject <- jsonlite::fromJSON(VariantSetResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(VariantSetResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(VariantSetResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- VariantSet$new()$fromJSON(jsonlite::toJSON(VariantSetResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
