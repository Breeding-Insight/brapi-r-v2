...
#' @export
PlateFormat <- R6::R6Class(
    "PlateFormat",
    public = list(
        initialize = function(...) {
            local.optional.var <- list(...)
            val <- unlist(local.optional.var)
            enumvec <- .parse_PlateFormat()
        ...
        fromJSON = function(PlateFormatJson) {
            private$value <- jsonlite::fromJSON(PlateFormatJson,
        ...
        fromJSONString = function(PlateFormatJson) {
            private$value <- jsonlite::fromJSON(PlateFormatJson,
                simplifyVector = FALSE)
            self
        }
...
# add to utils.R
.parse_PlateFormat <- function(vals) {
    res <- gsub("^\\[|\\]$", "",
        "[PLATE_96, TUBES]"
    )
    unlist(strsplit(res, ", "))
