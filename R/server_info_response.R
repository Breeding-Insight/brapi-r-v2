...
#'
#' @description ServerInfoResponse Class
#'
...
#'
#' @field result  \link{ServerInfo} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
ServerInfoResponse <- R6::R6Class(
  'ServerInfoResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      ServerInfoResponseObject <- list()
      if (!is.null(self$`@context`)) {
        ServerInfoResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        ServerInfoResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        ServerInfoResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      ServerInfoResponseObject
    },
    fromJSON = function(ServerInfoResponseJson) {
      ServerInfoResponseObject <- jsonlite::fromJSON(ServerInfoResponseJson)
      if (!is.null(ServerInfoResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(ServerInfoResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(ServerInfoResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(ServerInfoResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(ServerInfoResponseObject$`result`)) {
        resultObject <- ServerInfo$new()
        resultObject$fromJSON(jsonlite::toJSON(ServerInfoResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(ServerInfoResponseJson) {
      ServerInfoResponseObject <- jsonlite::fromJSON(ServerInfoResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(ServerInfoResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(ServerInfoResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- ServerInfo$new()$fromJSON(jsonlite::toJSON(ServerInfoResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
