...
#'
#' @description EventsResponse Class
#'
...
#'
#' @field result  \link{EventsResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
EventsResponse <- R6::R6Class(
  'EventsResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      EventsResponseObject <- list()
      if (!is.null(self$`@context`)) {
        EventsResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        EventsResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        EventsResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      EventsResponseObject
    },
    fromJSON = function(EventsResponseJson) {
      EventsResponseObject <- jsonlite::fromJSON(EventsResponseJson)
      if (!is.null(EventsResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(EventsResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(EventsResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(EventsResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(EventsResponseObject$`result`)) {
        resultObject <- EventsResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(EventsResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(EventsResponseJson) {
      EventsResponseObject <- jsonlite::fromJSON(EventsResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(EventsResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(EventsResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- EventsResponseResult$new()$fromJSON(jsonlite::toJSON(EventsResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
