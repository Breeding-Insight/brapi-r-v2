...
#'
#' @description SeedLotSingleResponse Class
#'
...
#'
#' @field result  \link{SeedLot} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
SeedLotSingleResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      SeedLotSingleResponseObject <- list()
      if (!is.null(self$`@context`)) {
        SeedLotSingleResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        SeedLotSingleResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        SeedLotSingleResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      SeedLotSingleResponseObject
    },
    fromJSON = function(SeedLotSingleResponseJson) {
      SeedLotSingleResponseObject <- jsonlite::fromJSON(SeedLotSingleResponseJson)
      if (!is.null(SeedLotSingleResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(SeedLotSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(SeedLotSingleResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(SeedLotSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(SeedLotSingleResponseObject$`result`)) {
        resultObject <- SeedLot$new()
        resultObject$fromJSON(jsonlite::toJSON(SeedLotSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(SeedLotSingleResponseJson) {
      SeedLotSingleResponseObject <- jsonlite::fromJSON(SeedLotSingleResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(SeedLotSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(SeedLotSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- SeedLot$new()$fromJSON(jsonlite::toJSON(SeedLotSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
