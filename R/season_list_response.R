...
#'
#' @description SeasonListResponse Class
#'
...
#'
#' @field result  \link{SeasonListResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
SeasonListResponse <- R6::R6Class(
  'SeasonListResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      SeasonListResponseObject <- list()
      if (!is.null(self$`@context`)) {
        SeasonListResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        SeasonListResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        SeasonListResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      SeasonListResponseObject
    },
    fromJSON = function(SeasonListResponseJson) {
      SeasonListResponseObject <- jsonlite::fromJSON(SeasonListResponseJson)
      if (!is.null(SeasonListResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(SeasonListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(SeasonListResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(SeasonListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(SeasonListResponseObject$`result`)) {
        resultObject <- SeasonListResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(SeasonListResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(SeasonListResponseJson) {
      SeasonListResponseObject <- jsonlite::fromJSON(SeasonListResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(SeasonListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(SeasonListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- SeasonListResponseResult$new()$fromJSON(jsonlite::toJSON(SeasonListResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
