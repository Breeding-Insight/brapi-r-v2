...
GeoJSON <- R6::R6Class(
  ...
  public = list(
    ...
    fromJSON = function(GeoJSONJson) {
      GeoJSONObject <- jsonlite::fromJSON(GeoJSONJson)
      if (!is.null(GeoJSONObject$`geometry`)) {
        geometryObject <- OneOfpointGeometrypolygonGeometry$new()
        geometryObject$fromJSON(jsonlite::toJSON(GeoJSONObject$geometry, auto_unbox = TRUE, digits = NA))
        self$`geometry` <- geometryObject
      }
      if (!is.null(GeoJSONObject$`type`)) {
    ...
    fromJSONString = function(GeoJSONJson) {
      GeoJSONObject <- jsonlite::fromJSON(GeoJSONJson)
      self$`geometry` <- OneOfpointGeometrypolygonGeometry$new()$fromJSON(jsonlite::toJSON(GeoJSONObject$geometry, auto_unbox = TRUE, digits = NA))
      self$`type` <- GeoJSONObject$`type`
      self
    }
  )
