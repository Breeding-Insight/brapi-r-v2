...
#'
#' @description CrossesListResponse Class
#'
...
#'
#' @field result  \link{CrossesListResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
CrossesListResponse <- R6::R6Class(
  'CrossesListResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      CrossesListResponseObject <- list()
      if (!is.null(self$`@context`)) {
        CrossesListResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        CrossesListResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        CrossesListResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      CrossesListResponseObject
    },
    fromJSON = function(CrossesListResponseJson) {
      CrossesListResponseObject <- jsonlite::fromJSON(CrossesListResponseJson)
      if (!is.null(CrossesListResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(CrossesListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(CrossesListResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(CrossesListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(CrossesListResponseObject$`result`)) {
        resultObject <- CrossesListResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(CrossesListResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(CrossesListResponseJson) {
      CrossesListResponseObject <- jsonlite::fromJSON(CrossesListResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(CrossesListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(CrossesListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- CrossesListResponseResult$new()$fromJSON(jsonlite::toJSON(CrossesListResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
