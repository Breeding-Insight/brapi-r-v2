...
#' @docType class
#' @title MetadataTokenPagination
#'
#' @description MetadataTokenPagination Class
#'
#' @format An \code{R6Class} generator object
#'
#' @field datafiles  list( \link{DataFile} ) [optional]
#'
#' @field status  list( \link{Status} ) [optional]
#'
#' @field pagination  \link{TokenPagination} [optional]
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
MetadataTokenPagination <- R6::R6Class(
  ...
  public = list(
    ...
    `pagination` = NULL,
    initialize = function(
        `datafiles`=NULL, `status`=NULL, `pagination`=NULL, ...
    ) {
    ...
    toJSON = function() {
      MetadataTokenPaginationObject <- list()
      if (!is.null(self$`datafiles`)) {
        MetadataTokenPaginationObject[['datafiles']] <-
          lapply(self$`datafiles`, function(x) x$toJSON())
      }
      if (!is.null(self$`status`)) {
        MetadataTokenPaginationObject[['status']] <-
          lapply(self$`status`, function(x) x$toJSON())
      }
      if (!is.null(self$`pagination`)) {
        MetadataTokenPaginationObject[['pagination']] <-
          self$`pagination`$toJSON()
      }

      MetadataTokenPaginationObject
    },
    fromJSON = function(MetadataTokenPaginationJson) {
      MetadataTokenPaginationObject <- jsonlite::fromJSON(MetadataTokenPaginationJson)
      if (!is.null(MetadataTokenPaginationObject$`datafiles`)) {
        self$`datafiles` <- ApiClient$new()$deserializeObj(MetadataTokenPaginationObject$`datafiles`, "array[DataFile]", loadNamespace("openapi"))
      }
      if (!is.null(MetadataTokenPaginationObject$`status`)) {
        self$`status` <- ApiClient$new()$deserializeObj(MetadataTokenPaginationObject$`status`, "array[Status]", loadNamespace("openapi"))
      }
      if (!is.null(MetadataTokenPaginationObject$`pagination`)) {
        paginationObject <- TokenPagination$new()
        paginationObject$fromJSON(jsonlite::toJSON(MetadataTokenPaginationObject$pagination, auto_unbox = TRUE, digits = NA))
...
',
    ...
    fromJSONString = function(MetadataTokenPaginationJson) {
      MetadataTokenPaginationObject <- jsonlite::fromJSON(MetadataTokenPaginationJson)
      self$`datafiles` <- ApiClient$new()$deserializeObj(MetadataTokenPaginationObject$`datafiles`, "array[DataFile]", loadNamespace("openapi"))
      self$`status` <- ApiClient$new()$deserializeObj(MetadataTokenPaginationObject$`status`, "array[Status]", loadNamespace("openapi"))
      self$`pagination` <- TokenPagination$new()$fromJSON(jsonlite::toJSON(MetadataTokenPaginationObject$pagination, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
