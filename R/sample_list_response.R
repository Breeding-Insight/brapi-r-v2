...
#'
#' @description SampleListResponse Class
#'
...
#'
#' @field result  \link{SampleListResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
SampleListResponse <- R6::R6Class(
  'SampleListResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      SampleListResponseObject <- list()
      if (!is.null(self$`@context`)) {
        SampleListResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        SampleListResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        SampleListResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      SampleListResponseObject
    },
    fromJSON = function(SampleListResponseJson) {
      SampleListResponseObject <- jsonlite::fromJSON(SampleListResponseJson)
      if (!is.null(SampleListResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(SampleListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(SampleListResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(SampleListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(SampleListResponseObject$`result`)) {
        resultObject <- SampleListResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(SampleListResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(SampleListResponseJson) {
      SampleListResponseObject <- jsonlite::fromJSON(SampleListResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(SampleListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(SampleListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- SampleListResponseResult$new()$fromJSON(jsonlite::toJSON(SampleListResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
