...
#'
#' @description ProgramListResponse Class
#'
...
#'
#' @field result  \link{ProgramListResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
ProgramListResponse <- R6::R6Class(
  'ProgramListResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      ProgramListResponseObject <- list()
      if (!is.null(self$`@context`)) {
        ProgramListResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        ProgramListResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        ProgramListResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      ProgramListResponseObject
    },
    fromJSON = function(ProgramListResponseJson) {
      ProgramListResponseObject <- jsonlite::fromJSON(ProgramListResponseJson)
      if (!is.null(ProgramListResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(ProgramListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(ProgramListResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(ProgramListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(ProgramListResponseObject$`result`)) {
        resultObject <- ProgramListResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(ProgramListResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(ProgramListResponseJson) {
      ProgramListResponseObject <- jsonlite::fromJSON(ProgramListResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(ProgramListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(ProgramListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- ProgramListResponseResult$new()$fromJSON(jsonlite::toJSON(ProgramListResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
