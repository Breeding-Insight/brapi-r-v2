...
#'
#' @description VendorOrderListResponse Class
#'
...
#'
#' @field result  \link{VendorOrderListResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
VendorOrderListResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      VendorOrderListResponseObject <- list()
      if (!is.null(self$`@context`)) {
        VendorOrderListResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        VendorOrderListResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        VendorOrderListResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      VendorOrderListResponseObject
    },
    fromJSON = function(VendorOrderListResponseJson) {
      VendorOrderListResponseObject <- jsonlite::fromJSON(VendorOrderListResponseJson)
      if (!is.null(VendorOrderListResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(VendorOrderListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(VendorOrderListResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(VendorOrderListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(VendorOrderListResponseObject$`result`)) {
        resultObject <- VendorOrderListResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(VendorOrderListResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(VendorOrderListResponseJson) {
      VendorOrderListResponseObject <- jsonlite::fromJSON(VendorOrderListResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(VendorOrderListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(VendorOrderListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- VendorOrderListResponseResult$new()$fromJSON(jsonlite::toJSON(VendorOrderListResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
