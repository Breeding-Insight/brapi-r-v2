...
#'
#' @description GermplasmSingleResponse Class
#'
...
#'
#' @field result  \link{Germplasm} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
GermplasmSingleResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      GermplasmSingleResponseObject <- list()
      if (!is.null(self$`@context`)) {
        GermplasmSingleResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        GermplasmSingleResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        GermplasmSingleResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      GermplasmSingleResponseObject
    },
    fromJSON = function(GermplasmSingleResponseJson) {
      GermplasmSingleResponseObject <- jsonlite::fromJSON(GermplasmSingleResponseJson)
      if (!is.null(GermplasmSingleResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(GermplasmSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(GermplasmSingleResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(GermplasmSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(GermplasmSingleResponseObject$`result`)) {
        resultObject <- Germplasm$new()
        resultObject$fromJSON(jsonlite::toJSON(GermplasmSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(GermplasmSingleResponseJson) {
      GermplasmSingleResponseObject <- jsonlite::fromJSON(GermplasmSingleResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(GermplasmSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(GermplasmSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- Germplasm$new()$fromJSON(jsonlite::toJSON(GermplasmSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
