...
#'
#' @description ListsListResponse Class
#'
...
#'
#' @field result  \link{ListsListResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
ListsListResponse <- R6::R6Class(
  'ListsListResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      ListsListResponseObject <- list()
      if (!is.null(self$`@context`)) {
        ListsListResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        ListsListResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        ListsListResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      ListsListResponseObject
    },
    fromJSON = function(ListsListResponseJson) {
      ListsListResponseObject <- jsonlite::fromJSON(ListsListResponseJson)
      if (!is.null(ListsListResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(ListsListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(ListsListResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(ListsListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(ListsListResponseObject$`result`)) {
        resultObject <- ListsListResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(ListsListResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(ListsListResponseJson) {
      ListsListResponseObject <- jsonlite::fromJSON(ListsListResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(ListsListResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(ListsListResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- ListsListResponseResult$new()$fromJSON(jsonlite::toJSON(ListsListResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
