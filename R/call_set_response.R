...
#'
#' @description CallSetResponse Class
#'
...
#'
#' @field result  \link{CallSet} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
CallSetResponse <- R6::R6Class(
  'CallSetResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      CallSetResponseObject <- list()
      if (!is.null(self$`@context`)) {
        CallSetResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        CallSetResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        CallSetResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      CallSetResponseObject
    },
    fromJSON = function(CallSetResponseJson) {
      CallSetResponseObject <- jsonlite::fromJSON(CallSetResponseJson)
      if (!is.null(CallSetResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(CallSetResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(CallSetResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(CallSetResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(CallSetResponseObject$`result`)) {
        resultObject <- CallSet$new()
        resultObject$fromJSON(jsonlite::toJSON(CallSetResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(CallSetResponseJson) {
      CallSetResponseObject <- jsonlite::fromJSON(CallSetResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(CallSetResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(CallSetResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- CallSet$new()$fromJSON(jsonlite::toJSON(CallSetResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
