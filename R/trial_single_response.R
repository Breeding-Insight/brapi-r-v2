...
#'
#' @description TrialSingleResponse Class
#'
...
#'
#' @field result  \link{Trial} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
TrialSingleResponse <- R6::R6Class(
  'TrialSingleResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      TrialSingleResponseObject <- list()
      if (!is.null(self$`@context`)) {
        TrialSingleResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        TrialSingleResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        TrialSingleResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      TrialSingleResponseObject
    },
    fromJSON = function(TrialSingleResponseJson) {
      TrialSingleResponseObject <- jsonlite::fromJSON(TrialSingleResponseJson)
      if (!is.null(TrialSingleResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(TrialSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(TrialSingleResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(TrialSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(TrialSingleResponseObject$`result`)) {
        resultObject <- Trial$new()
        resultObject$fromJSON(jsonlite::toJSON(TrialSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(TrialSingleResponseJson) {
      TrialSingleResponseObject <- jsonlite::fromJSON(TrialSingleResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(TrialSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(TrialSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- Trial$new()$fromJSON(jsonlite::toJSON(TrialSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
