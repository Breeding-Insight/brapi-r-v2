...
#'
#' @description MethodSingleResponse Class
#'
...
#'
#' @field result  \link{Method} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
MethodSingleResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      MethodSingleResponseObject <- list()
      if (!is.null(self$`@context`)) {
        MethodSingleResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        MethodSingleResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        MethodSingleResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      MethodSingleResponseObject
    },
    fromJSON = function(MethodSingleResponseJson) {
      MethodSingleResponseObject <- jsonlite::fromJSON(MethodSingleResponseJson)
      if (!is.null(MethodSingleResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(MethodSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(MethodSingleResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(MethodSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(MethodSingleResponseObject$`result`)) {
        resultObject <- Method$new()
        resultObject$fromJSON(jsonlite::toJSON(MethodSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(MethodSingleResponseJson) {
      MethodSingleResponseObject <- jsonlite::fromJSON(MethodSingleResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(MethodSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(MethodSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- Method$new()$fromJSON(jsonlite::toJSON(MethodSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
