...
#'
#' @description VariantSingleResponse Class
#'
...
#'
#' @field result  \link{Variant} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
VariantSingleResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      VariantSingleResponseObject <- list()
      if (!is.null(self$`@context`)) {
        VariantSingleResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        VariantSingleResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        VariantSingleResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      VariantSingleResponseObject
    },
    fromJSON = function(VariantSingleResponseJson) {
      VariantSingleResponseObject <- jsonlite::fromJSON(VariantSingleResponseJson)
      if (!is.null(VariantSingleResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(VariantSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(VariantSingleResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(VariantSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(VariantSingleResponseObject$`result`)) {
        resultObject <- Variant$new()
        resultObject$fromJSON(jsonlite::toJSON(VariantSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(VariantSingleResponseJson) {
      VariantSingleResponseObject <- jsonlite::fromJSON(VariantSingleResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(VariantSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(VariantSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- Variant$new()$fromJSON(jsonlite::toJSON(VariantSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
