...
#' @title LinkageGroup
#'
#' @description LinkageGroup Class
#'
...
LinkageGroup <- R6::R6Class(
  ...
  public = list(
    ...
    `maxPosition` = NULL,
    initialize = function(
        `additionalInfo`=NULL, `linkageGroupName`=NULL, `markerCount`=NULL, `maxPosition`=NULL, ...
    ) {
    ...
    toJSON = function() {
      LinkageGroupObject <- list()
      if (!is.null(self$`additionalInfo`)) {
        LinkageGroupObject[['additionalInfo']] <-
          self$`additionalInfo`
      }
      if (!is.null(self$`linkageGroupName`)) {
        LinkageGroupObject[['linkageGroupName']] <-
          self$`linkageGroupName`
      }
      if (!is.null(self$`markerCount`)) {
        LinkageGroupObject[['markerCount']] <-
          self$`markerCount`
      }
      if (!is.null(self$`maxPosition`)) {
        LinkageGroupObject[['maxPosition']] <-
          self$`maxPosition`
      }

      LinkageGroupObject
    },
    fromJSON = function(LinkageGroupJson) {
      LinkageGroupObject <- jsonlite::fromJSON(LinkageGroupJson)
      if (!is.null(LinkageGroupObject$`additionalInfo`)) {
        self$`additionalInfo` <- ApiClient$new()$deserializeObj(LinkageGroupObject$`additionalInfo`, "map(character)", loadNamespace("openapi"))
      }
      if (!is.null(LinkageGroupObject$`linkageGroupName`)) {
        self$`linkageGroupName` <- LinkageGroupObject$`linkageGroupName`
      }
      if (!is.null(LinkageGroupObject$`markerCount`)) {
        self$`markerCount` <- LinkageGroupObject$`markerCount`
      }
      if (!is.null(LinkageGroupObject$`maxPosition`)) {
        self$`maxPosition` <- LinkageGroupObject$`maxPosition`
      }
      self
    },
    ...
    fromJSONString = function(LinkageGroupJson) {
      LinkageGroupObject <- jsonlite::fromJSON(LinkageGroupJson)
      self$`additionalInfo` <- ApiClient$new()$deserializeObj(LinkageGroupObject$`additionalInfo`, "map(character)", loadNamespace("openapi"))
      self$`linkageGroupName` <- LinkageGroupObject$`linkageGroupName`
      self$`markerCount` <- LinkageGroupObject$`markerCount`
