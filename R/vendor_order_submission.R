...
#' @docType class
#' @title VendorOrderSubmission
#'
#' @description VendorOrderSubmission Class
#'
#' @format An \code{R6Class} generator object
#'
#' @field orderId  character 
#'
#' @field shipmentForms  list( \link{ShipmentForm} ) [optional]
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
VendorOrderSubmission <- R6::R6Class(
  'VendorOrderSubmission',
  public = list(
    `orderId` = NULL,
    `shipmentForms` = NULL,
    initialize = function(
        `orderId`, `shipmentForms`=NULL, ...
    ) {
      ...
      if (!is.null(`shipmentForms`)) {
        stopifnot(is.vector(`shipmentForms`), length(`shipmentForms`) != 0)
        sapply(`shipmentForms`, function(x) stopifnot(R6::is.R6(x)))
        self$`shipmentForms` <- `shipmentForms`
      }
    },
    toJSON = function() {
      VendorOrderSubmissionObject <- list()
      if (!is.null(self$`orderId`)) {
        VendorOrderSubmissionObject[['orderId']] <-
          self$`orderId`
      }
      if (!is.null(self$`shipmentForms`)) {
        VendorOrderSubmissionObject[['shipmentForms']] <-
          lapply(self$`shipmentForms`, function(x) x$toJSON())
      }

      VendorOrderSubmissionObject
    },
    fromJSON = function(VendorOrderSubmissionJson) {
      VendorOrderSubmissionObject <- jsonlite::fromJSON(VendorOrderSubmissionJson)
      if (!is.null(VendorOrderSubmissionObject$`orderId`)) {
        self$`orderId` <- VendorOrderSubmissionObject$`orderId`
      }
      if (!is.null(VendorOrderSubmissionObject$`shipmentForms`)) {
        self$`shipmentForms` <- ApiClient$new()$deserializeObj(VendorOrderSubmissionObject$`shipmentForms`, "array[ShipmentForm]", loadNamespace("openapi"))
      }
      self
    },
    toJSONString = function() {
      jsoncontent <- c(
        ...
        [%s]
',
        paste(sapply(self$`shipmentForms`, function(x) jsonlite::toJSON(x$toJSON(), auto_unbox=TRUE, digits = NA)), collapse=",")
        )}
      )
      jsoncontent <- paste(jsoncontent, collapse = ",")
      paste('{', jsoncontent, '}', sep = "")
    },
    fromJSONString = function(VendorOrderSubmissionJson) {
      VendorOrderSubmissionObject <- jsonlite::fromJSON(VendorOrderSubmissionJson)
      self$`orderId` <- VendorOrderSubmissionObject$`orderId`
      self$`shipmentForms` <- ApiClient$new()$deserializeObj(VendorOrderSubmissionObject$`shipmentForms`, "array[ShipmentForm]", loadNamespace("openapi"))
      self
