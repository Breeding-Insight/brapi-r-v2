...
Measurement <- R6::R6Class(
  ...
  public = list(
    ...
    fromJSON = function(MeasurementJson) {
      ...
      if (!is.null(MeasurementObject$`units`)) {
        self$`units` <- MeasurementObject$`units`
      }
      if (!is.null(MeasurementObject$`value`)) {
        self$`value` <- MeasurementObject$`value`
      }
      self
    },
