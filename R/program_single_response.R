...
#'
#' @description ProgramSingleResponse Class
#'
...
#'
#' @field result  \link{Program} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
ProgramSingleResponse <- R6::R6Class(
  ...
  public = list(
    ...
    toJSON = function() {
      ProgramSingleResponseObject <- list()
      if (!is.null(self$`@context`)) {
        ProgramSingleResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        ProgramSingleResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        ProgramSingleResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      ProgramSingleResponseObject
    },
    fromJSON = function(ProgramSingleResponseJson) {
      ProgramSingleResponseObject <- jsonlite::fromJSON(ProgramSingleResponseJson)
      if (!is.null(ProgramSingleResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(ProgramSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(ProgramSingleResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(ProgramSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(ProgramSingleResponseObject$`result`)) {
        resultObject <- Program$new()
        resultObject$fromJSON(jsonlite::toJSON(ProgramSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(ProgramSingleResponseJson) {
      ProgramSingleResponseObject <- jsonlite::fromJSON(ProgramSingleResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(ProgramSingleResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(ProgramSingleResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- Program$new()$fromJSON(jsonlite::toJSON(ProgramSingleResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
