...
#'
#' @description StudyTypesResponse Class
#'
...
#'
#' @field result  \link{StudyTypesResponseResult} 
#'
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
StudyTypesResponse <- R6::R6Class(
  'StudyTypesResponse',
  public = list(
    `@context` = NULL,
    `metadata` = NULL,
    ...
    toJSON = function() {
      StudyTypesResponseObject <- list()
      if (!is.null(self$`@context`)) {
        StudyTypesResponseObject[['@context']] <-
          self$`@context`
      }
      if (!is.null(self$`metadata`)) {
        StudyTypesResponseObject[['metadata']] <-
          self$`metadata`$toJSON()
      }
      if (!is.null(self$`result`)) {
        StudyTypesResponseObject[['result']] <-
          self$`result`$toJSON()
      }

      StudyTypesResponseObject
    },
    fromJSON = function(StudyTypesResponseJson) {
      StudyTypesResponseObject <- jsonlite::fromJSON(StudyTypesResponseJson)
      if (!is.null(StudyTypesResponseObject$`@context`)) {
        self$`@context` <- ApiClient$new()$deserializeObj(StudyTypesResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      }
      if (!is.null(StudyTypesResponseObject$`metadata`)) {
        metadataObject <- Metadata$new()
        metadataObject$fromJSON(jsonlite::toJSON(StudyTypesResponseObject$metadata, auto_unbox = TRUE, digits = NA))
        self$`metadata` <- metadataObject
      }
      if (!is.null(StudyTypesResponseObject$`result`)) {
        resultObject <- StudyTypesResponseResult$new()
        resultObject$fromJSON(jsonlite::toJSON(StudyTypesResponseObject$result, auto_unbox = TRUE, digits = NA))
    ...
    fromJSONString = function(StudyTypesResponseJson) {
      StudyTypesResponseObject <- jsonlite::fromJSON(StudyTypesResponseJson)
      self$`@context` <- ApiClient$new()$deserializeObj(StudyTypesResponseObject$`@context`, "array[character]", loadNamespace("openapi"))
      self$`metadata` <- Metadata$new()$fromJSON(jsonlite::toJSON(StudyTypesResponseObject$metadata, auto_unbox = TRUE, digits = NA))
      self$`result` <- StudyTypesResponseResult$new()$fromJSON(jsonlite::toJSON(StudyTypesResponseObject$result, auto_unbox = TRUE, digits = NA))
      self
    }
  )
)
